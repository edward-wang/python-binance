# Section 6: Exception Hierarchy (`_core/exceptions.py`)

Structured error handling with Binance error code mapping.
...
BinanceError (base)
│
├── BinanceNetworkError (+ maybe_executed, method, path)
│   ├── ConnectionError
│   └── TimeoutError
│
└── BinanceAPIError (+ enhanced __repr__)
    │
    ├── RateLimitError (+ is_order_limit property)
    ├── IPBannedError
...
class BinanceNetworkError(BinanceError):
    ...
    def __init__(self, message: str, method: str = "GET", path: str = ""):
        self.method = method
        self.path = path
...
class BinanceAPIError(BinanceError):
    ...
    def __init__(self, code: int, message: str, meta: APIErrorMeta | None = None):
        self.code = code
...
try:
    ...
    if e.is_order_limit:
        await strategy.pause_symbol(symbol)  # Serious
    else:
        await asyncio.sleep(e.retry_after or 1)  # Just weight
except TimeoutError as e:
    if e.maybe_executed:
        # DANGER: Do NOT retry! Check order status first
        existing = await client.get_order(symbol=symbol, client_order_id=order_id)
        if existing:
            return existing
